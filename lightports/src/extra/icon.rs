...
impl Icon {
    pub fn create<'t>(
        ...
        let ret = match size {
            ResourceSize::Small => unsafe {
                LoadIconMetric(hinst, name, LIM_SMALL as i32, &mut icon_ret);
                icon_ret
            },
            ResourceSize::Large => unsafe {
                LoadIconMetric(hinst, name, LIM_LARGE as i32, &mut icon_ret);
                icon_ret
            },
            ResourceSize::Resource => unsafe {
                LoadImageW(hinst, name, IMAGE_ICON, 0, 0, flags) as HICON
            },
            ResourceSize::Specific(w, h) => unsafe {
                LoadImageW(hinst, name, IMAGE_ICON, w, h, flags) as HICON
            },
        };
